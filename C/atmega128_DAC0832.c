...
#include <stdio.h>
#include <delay.h>
#define CS_Bar PORTD.4      //ADA DI PORT D4
...
void send2DAC(unsigned char x)
    ...
    DATA_PORT=x;
    WR_Bar=1;
    WR_Bar=0;   //buat delay_us kalau belum cukup clock nya
    WR_Bar=1;
    CS_Bar=1;
    }
...
while (1)
      ...
      for (data=0;data<=255;data++) //
        {
        send2DAC(data);
        sprintf(text,"Data=%-3d", data);    //rata kiri dengan 3 digit. kalau kanan ga pake -
        lcd_gotoxy(0,0);
        lcd_puts(text);
